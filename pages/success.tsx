...
const Success = ({products}: Props) => {
    ...
    const [showOrderSumary, setShowOrderSumary] = useState(false)

    useEffect(() => {
        setMounted(true)
    }, [])

    // showOrderSumary always true for desktop but only conditionally true for mobile
    const isTabletOrMobile = useMediaQuery({ query: "(maxWidth: 1024px)" })

    const showOrderSummaryCondition = isTabletOrMobile ? showOrderSumary : true

    const handleShowOrderSummary = () => {
        setShowOrderSumary(!showOrderSumary)
    }
    ...
    return (
        <div>
            ...
            <main className="grid grid-cols-1 lg:grid-cols-9">
                <section className="order-2 mx-auto max-w-xl pb-12 lg:col-span-5 lg:mx-0 lg:max-w-none lg:pr-16 lg:pt-16 xl:pl-16 2xl:pt-44">
                    <Link href="/">
                    ...
                    <div className="mx-4 divide-y divide-gray-300 rounded-md boder border-gray-300 p-4 lg:ml-14">
                        <div className="space-y-2 pb-3">
                            <p>
                                Your order is conirmed
                            </p>
                            <p className="text-sm text-gray-600">
                                                    ...
                                                    >
                                                        <div className="relative flex h-16 w-16 items-center justify-center rounded-md border border-gray-300 bg-[#F1F1F1] text-xs text-white">
                                                            <div className="relative h-7 w-7 animate-bounce rounded-md">
                                                                <Image 
                                                                    src="https://rb.gy/vsvv20" layout='fill' 
                                                                    objectFit='contain'
                                                                />
                                                            </div>
...
export const getServerSideProps: GetServerSideProps<Props> = async ({query}) => {
    ...
    const sesstionId = query.session_id as string
    const products =  await fetchLineItems(sesstionId)
